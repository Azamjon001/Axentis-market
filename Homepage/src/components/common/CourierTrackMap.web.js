...
// 🌐 Веб-версия карты курьера: тот же Leaflet, но в <iframe> вместо WebView
// (react-native-webview на вебе не работает). Логика и вид идентичны нативу.
function buildHtml(courier, dest) {
  ...
  <script>
    var map = L.map('map', { zoomControl: true }).setView([${cLat}, ${cLng}], 14);
...
export default function CourierTrackMap({ courier, destination, style }) {
  ...
  useEffect(() => {
    if (courier?.lat != null && courier?.lng != null && iframeRef.current?.contentWindow) {
      iframeRef.current.contentWindow.postMessage(
        JSON.stringify({ type: 'courier', lat: courier.lat, lng: courier.lng }),
        '*',
      );
    }
  ...
  return (
    <View style={[styles.wrap, style]}>
      <iframe
        ref={iframeRef}
        srcDoc={html}
        style={{ width: '100%', height: '100%', border: 'none', backgroundColor: '#e9eef2' }}
        title="courier-map"
      />
    </View>
  );
