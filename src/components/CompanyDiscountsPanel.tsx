...
interface CompanyDiscountsPanelProps {
  companyId: number;
  products?: any[];
}

export default function CompanyDiscountsPanel({ companyId, products: initialProducts = [] }: CompanyDiscountsPanelProps) {
  const [discounts, setDiscounts] = useState<Discount[]>([]);
  const [aggressiveDiscounts, setAggressiveDiscounts] = useState<any[]>([]);
  ...
  const [formData, setFormData] = useState({
    productId: '',
    variantId: '', // '' = весь товар (любой вариант), иначе конкретный SKU
    discountPercent: '',
    title: '',
    description: '',
    startDate: '',
  ...
  // Загружаем SKU-варианты выбранного товара, чтобы скидку можно было
  // навесить либо на весь товар, либо на конкретный вариант.
  ...
  const getUsedProductIds = () => {
    const regularIds = discounts.map(d => d.productId);
    const aggressiveIds = aggressiveDiscounts.map(d => d.productId);
  ...
  const handleCreateDiscount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ productId: '', variantId: '', discountPercent: '', title: '', description: '', startDate: '', endDate: '' });
      setVariants([]);
      setShowForm(false);
      fetchDiscounts();
    } catch (error: any) {
  ...
  return (
    <div className="discounts-panel-container" style={styles.container}>
      <div style={styles.header}>
        <div>
          <h2 style={styles.title}>🏷️ {t.regularDiscounts}</h2>
          <p className="discount-subtitle" style={styles.subtitle}>{t.discountOnMarkupOnly}</p>
        </div>
        <button 
          style={styles.createButton} 
          onClick={() => setShowForm(!showForm)}
        >
          {showForm ? `✖ ${t.closeBtn}` : `➕ ${t.createDiscountBtn}`}
        </button>
      </div>
      ...
      <div style={styles.discountsList}>
        {loading ? (
          ...
          <div style={styles.empty}>
            ...
            <p style={styles.emptyHint}>{t.createFirstDiscount}</p>
          </div>
        ) : (
          discounts.map(discount => {
            ...
            // Calculate discount on markup only
            // productBasePrice = базовая цена, productPrice = цена с наценкой (selling price)
                ...
                <div style={styles.cardBody}>
                  ...
                  <div style={styles.cardContent}>
                    ...
                    <div className="discount-dates" style={styles.dates}>
                      {t.createdOn} {new Date(discount.createdAt).toLocaleDateString('uz-UZ')}
                    </div>
                  </div>
                </div>
              </div>
